...
* 创建一个可编辑区
*/
listTable.edit = function(url, obj, id)
{
	...
	txt.onblur = function(e)
	{
		if (Utils.trim(txt.value).length > 0)
		{
			...
			jQuery.get(url, "val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id, function(data){
				if (data.message)
				{
					alert(data.message);
				}
...
listTable.addvalue = function(obj, act, id)
{
	/* 保存原始的内容 */
	var org = obj.innerHTML;

	res = Ajax.call(listTable.url, "act="+ act + "&id=" + id, null, "GET", "JSON", false);
	if (res.message)
	{
		alert(res.message);
	}
	obj.innerHTML = (res.error == 0 || res.error == '') ? res.content : org;
}

/**
* 切换状态
*/
listTable.toggle = function(url, obj, id)
{
	var val = (obj.src.match(/yes.gif/i)) ? 0 : 1;

	var res = jQuery.get(url, "val=" + val + "&id=" +id, function(data){
		if (data.error == 0)
		{
			obj.src = (data.content > 0) ? img_dir + '/yes.gif' : img_dir + '/no.gif';
		}
		else
		{
			alert(data.message);
...
* 切换状态(安全模式)
*/
listTable.toggle_secure = function(url, obj, id)
{
	if (!confirm('确定要改变状态吗？')){
		return false;
	}
...
* 删除列表中的一个记录
*/
listTable.remove = function(url, cfm, id)
{
	if (confirm(cfm))
	{
...
listTable.getPageSize = function()
{
	...
	if (pageSize)
	{
		...
		document.cookie = "EZCP[page_size]=" + ps + ";";
	}
}
